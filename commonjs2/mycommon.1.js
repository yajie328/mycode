...
Module.getModulePaths =  function(filename){ // 在node_modules模块中读取文件
   let dir = path.parse(filename).dir; // 所在目录
   let base = path.parse(filename).base; // 
   let dirs = dir.split(path.sep); // 分割目录
   
   let parentDirs = []
   for(let i=0; i<dirs.length; i++){
       let curPath = path.join(dirs.slice(0, dirs.length-i).join(path.sep) ,'node_modules', base);
       parentDirs.push(curPath);
   }
    return parentDirs;
}
